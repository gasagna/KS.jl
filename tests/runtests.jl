...
@testset "spaces                                 " begin
    @testset "FTField                                " begin
        # constructors
        @test fieldsize(FTField([0, im], 1))     == 1
        @test fieldsize(FTField([0, im, im], 1)) == 2
        # indexing
        uk = FTField(2, 1)
    ...
    @testset "Field                                  " begin
        ...
        # provide data, must be odd
        v = Field([0, 1, 2], 1)
        @test fieldsize(v) == 1
        v = Field([0, 1, 2, 3, 4], 1)
        @test fieldsize(v) == 2
        v = Field([0, 1, 2, 3, 4, 5, 6], 1)
        @test fieldsize(v) == 3
        @test_throws ArgumentError Field([0, 1, 2, 3], 1)
    end
end
...
@testset "shift                                  " begin
    ...
    u = Field(fun.(x), 2)
    uk = ForwardFFT(similar(u))(u, FTField(3, 2))
    shift!(uk, 2/7) # shift left by one grid point 
    v = InverseFFT(similar(uk))(uk, Field(3, 2))
